...
public class TestLocalFileSystemPermission {
   ...
   */
  @Fuzz
  public void testSetUmaskInRealTimeFuzz(@From(ConfigurationGenerator.class) Configuration generatedConfig) throws Exception {
    assumeNotWindows();

    LocalFileSystem localfs = FileSystem.getLocal(new Configuration(generatedConfig));
    Configuration conf = localfs.getConf();
